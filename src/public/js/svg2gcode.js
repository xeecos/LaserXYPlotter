function svg2gcode(svg, settings) {
  ...
  settings.power = settings.power || 80;
  settings.rotate = settings.rotate || false;
  ...
  for (var pathIdx = 0, pathLength = paths.length; pathIdx < pathLength; pathIdx++) {
    ...
    if(settings.mode=="axidraw"){
      ...
      gcode.push(['G1',
        (settings.rotate?'Y':'X') + scale(path[0].x),
        (settings.rotate?('X'+(-scale(path[0].y))):('Y'+scale(path[0].y))),
        'F' + settings.feedRate
      ].join(' '));
      gcode.push('M3 P'+settings.power);
    }
    for (var p = settings.passWidth; p<=settings.materialWidth; p+=settings.passWidth) {
      ...
      for (var segmentIdx=0, segmentLength = path.length; segmentIdx<segmentLength; segmentIdx++) {
        ...
        if(settings.mode=="axidraw"){
          ...
          localSegment = ['G1',
              (settings.rotate?'Y':'X') + scale(segment.x),
              (settings.rotate?('X'+(-scale(segment.y))):('Y'+scale(segment.y))),
              'F' + settings.feedRate
            ].join(' ');
        }
